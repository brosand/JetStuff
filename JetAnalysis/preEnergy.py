...
#index will give the particle index, eg [7, 21, 32], k gives 0, 1, 2
    ...
    # print("energy[maxm] for jet %d: %f" % (k, pEvent.energy[maxm]))
    # print("energy[submax] for jet %d: %f" % (k, pEvent.energy[submax]))


    histsubmaxcheck.Fill(pEvent.energy[maxm], pEvent.energy[submax])
...
def readTree(filename1, filename2, fileOut, dimension, collisionType, folder):
    ...
    histsubmaxcheck.GetXaxis().SetTitle("energy of max");
    histsubmaxcheck.GetYaxis().SetTitle("energy of submax");
...
#LOOP: through each event in tree
    for pEvent, jEvent in  izip(tree, jetTree): #zip
        # if (iEvent==1):
        #     break
        sys.stdout.write("\rEvent %d: " % iEvent)
        sys.stdout.flush()
...
# 
    ...
    canvasBefore.SaveAs(folder + "/" + filename1.split('.')[0] + "_" + str(dimension) + "_before_e.pdf")

    canvasCentre.cd()
    histCentre.Draw("lego")
    #https://root.cern.ch/root/html534/guides/users-guide/Histograms.html
    canvasCentre.SaveAs(folder + "/" + filename1.split('.')[0] + "_" + str(dimension) + "_centre_e.pdf")

    canvasRotate.cd()
    histRotate.Draw("lego")
    canvasRotate.SaveAs(folder + "/" + filename1.split('.')[0] + "_" + str(dimension) + "_rotate_e.pdf")

    canvasTranslate.cd()
    histTranslate.Draw("lego")
    canvasTranslate.SaveAs(folder + "/" + filename1.split('.')[0] + "_" + str(dimension) + "_translate_e.pdf")

    canvasReflect.cd()
    histReflect.Draw("lego")
    canvasReflect.SaveAs(folder + "/" + filename1.split('.')[0] + "_" + str(dimension) + "_reflect_e.pdf")

    canvasSubmaxCheck.cd()
    histsubmaxcheck.Draw("lego")
    canvasSubmaxCheck.SaveAs(folder + "/" + filename1.split('.')[0] + "_" + str(dimension) + "_submaxCheck_e.pdf")
...
#give tree, jettree, output txt, collisiontype= , dimension=         
    ...
    fileOut = folder + "/" + filename2.split('/')[1].split('.')[0] + "Pre" + str(dimension) + "_e.txt"
